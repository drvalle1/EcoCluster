...
data(data_sam) 
```
The objects `data_mixture`, `data_sbm`, and `data_sam` contain presence/absence data for different locations (rows) and species (columns). For SAM, we also need data on covariates, which are stored in the object `xmat`, where rows are different locations and columns correspond to different covariates. 
